/**
 * Shared constants and element resolution logic used by both the local
 * MCP server (server.ts) and the remote MCP server (remote-server.ts).
 */
...
export const PSEUDO_TYPES = new Set(["cameraUpdate", "delete", "restoreCheckpoint"]);

/**
 * Result of resolving element references (checkpoint restore + deletes).
 */
export type ResolveResult =
  | { ok: true; resolvedElements: any[]; ratioHint: string }
  | { ok: false; error: string };

/**
 * Resolve restoreCheckpoint references and delete pseudo-elements.
 * Returns the fully resolved element array ready for rendering/storage.
 *
 * @param parsed - Parsed element array (may contain pseudo-elements).
 * @param store - CheckpointStore for loading saved state.
 * @returns Resolved elements and camera ratio hint, or an error message.
 */
...
/**
 * Generate a short checkpoint ID from a UUID.
 *
 * @returns 18-character alphanumeric string.
 */
